...
import com.legacy.system.datetime.LocalDate;
import com.legacy.system.datetime.LocalDateTime;
import com.legacy.system.datetime.LocalTime;
import com.legacy.system.datetime.MutableDateTime;
import com.legacy.system.datetime.ReadWritableInstant;
 ...
 *
 * <p>The underlying printer/parser can be altered to behave exactly as required by using one of the
 * decorator modifiers:
 *
 * <ul>
 ...
 *   <li>{@link #withChronology(Chronology)} - returns a new formatter that uses the specified
 *       chronology
 *   <li>{@link #withOffsetParsed()} - returns a new formatter that returns the parsed time zone
 *       offset
 ...
 * </pre>
 *
 * <p>Parsing builds up the resultant instant by 'setting' the value of each parsed field from
 * largest to smallest onto an initial instant, typically 1970-01-01T00:00Z. This design means that
 * day-of-month is set before day-of-week. As such, if both the day-of-month and day-of-week are
 * parsed, and the day-of-week is incorrect, then the day-of-week overrides the day-of-month.
 *
 * <p>This has a side effect if the input is not consistent.
 *
...
public class DateTimeFormatter {
   ...
   *
   * <p>A DateTimeFormatter is immutable, so a new instance is returned, and the original is
   * unaltered and still usable.
   *
  ...
  public DateTimeFormatter withLocale(Locale locale) {
    ...
    return new DateTimeFormatter(
        iPrinter, iParser, locale, iOffsetParsed, iChrono, iZone, iPivotYear, iDefaultYear);
  }
  ...
  // -----------------------------------------------------------------------
  /**
   * Returns a new formatter that will create a datetime with a time zone equal to that of the
   * offset of the parsed string.
   *
   * <p>After calling this method, a string '2004-06-09T10:20:30-08:00' will create a datetime with
   * a zone of -08:00 (a fixed zone, with no daylight savings rules). If the parsed string
   * represents a local time (no zone offset) the parsed datetime will be in the default zone.
   *
   * <p>Calling this method sets the override zone to null. Calling the override zone method sets
   * this flag off.
   *
   * @return the new formatter
   */
  public DateTimeFormatter withOffsetParsed() {
    ...
    return new DateTimeFormatter(
        iPrinter, iParser, iLocale, true, iChrono, null, iPivotYear, iDefaultYear);
  }
  ...
  // -----------------------------------------------------------------------
  /**
   * Returns a new formatter that will use the specified chronology in preference to that of the
   * printed object, or ISO on a parse.
   *
   * <p>When printing, this chronology will be used in preference to the chronology from the
   * datetime that would otherwise be used.
   *
   * <p>When parsing, this chronology will be set on the parsed datetime.
   *
   * <p>A null chronology means no-override. If both an override chronology and an override zone are
   * set, the override zone will take precedence over the zone in the chronology.
   *
  ...
  public DateTimeFormatter withChronology(Chronology chrono) {
    ...
    return new DateTimeFormatter(
        iPrinter, iParser, iLocale, iOffsetParsed, chrono, iZone, iPivotYear, iDefaultYear);
  }
  ...
  // -----------------------------------------------------------------------
  /**
   * Returns a new formatter that will use the UTC zone in preference to the zone of the printed
   * object, or default zone on a parse.
   *
   * <p>When printing, UTC will be used in preference to the zone from the datetime that would
   * otherwise be used.
   *
   * <p>When parsing, UTC will be set on the parsed datetime.
   *
   * <p>If both an override chronology and an override zone are set, the override zone will take
   * precedence over the zone in the chronology.
   *
  ...
  /**
   * Returns a new formatter that will use the specified zone in preference to the zone of the
   * printed object, or default zone on a parse.
   *
   * <p>When printing, this zone will be used in preference to the zone from the datetime that would
   * otherwise be used.
   *
   * <p>When parsing, this zone will be set on the parsed datetime.
   *
   * <p>A null zone means of no-override. If both an override chronology and an override zone are
   * set, the override zone will take precedence over the zone in the chronology.
   *
  ...
  public DateTimeFormatter withZone(DateTimeZone zone) {
    ...
    return new DateTimeFormatter(
        iPrinter, iParser, iLocale, false, iChrono, zone, iPivotYear, iDefaultYear);
  }
  ...
  // -----------------------------------------------------------------------
  /**
   * Returns a new formatter that will use the specified pivot year for two digit year parsing in
   * preference to that stored in the parser.
   *
   * <p>This setting is useful for changing the pivot year of formats built using a pattern - {@link
   * DateTimeFormat#forPattern(String)}.
   *
   * <p>When parsing, this pivot year is used. Null means no-override. There is no effect when
   * printing.
   *
   * <p>The pivot year enables a two digit year to be converted to a four digit year. The pivot
   * represents the year in the middle of the supported range of years. Thus the full range of years
   * that will be built is <code>(pivot - 50) .. (pivot + 49)</code>.
   *
   * <pre>
  ...
  public DateTimeFormatter withPivotYear(Integer pivotYear) {
    ...
    return new DateTimeFormatter(
        iPrinter, iParser, iLocale, iOffsetParsed, iChrono, iZone, pivotYear, iDefaultYear);
  }
   ...
   * DateTimeFormat#forPattern(String)}.
   *
   * <p>When parsing, this pivot year is used. There is no effect when printing.
   *
   * <p>The pivot year enables a two digit year to be converted to a four digit year. The pivot
   * represents the year in the middle of the supported range of years. Thus the full range of years
   * that will be built is <code>(pivot - 50) .. (pivot + 49)</code>.
   ...
   *
   * <p>The default year is used when parsing in the case where there is a month or a day but not a
   * year. Specifically, it is used if there is a field parsed with a duration between the length of
   * a month and the length of a day inclusive.
   *
   * <p>This value is typically used to move the year from 1970 to a leap year to enable February
   * 29th to be parsed. Unless customised, the year 2000 is used.
   *
   * <p>This setting has no effect when printing.
   *
  ...
  public DateTimeFormatter withDefaultYear(int defaultYear) {
    return new DateTimeFormatter(
        iPrinter, iParser, iLocale, iOffsetParsed, iChrono, iZone, iPivotYear, defaultYear);
  }
  ...
  // -----------------------------------------------------------------------
  /**
   * Prints an instant from milliseconds since 1970-01-01T00:00:00Z, using ISO chronology in the
   ...
   *
   * <p>This method will use the override zone and the override chronology if they are set.
   * Otherwise it will use the chronology and zone of the instant.
   *
   * @param instant instant to format, null means now
   ...
   *
   * <p>This method will use the override zone and the override chronology if they are set.
   * Otherwise it will use the ISO chronology and default zone.
   *
   * @param instant millis since 1970-01-01T00:00:00Z
   * @return the printed result
   */
   ...
   *
   * <p>The parse will use the ISO chronology, and the default time zone. If the text contains a
   * time zone string then that will be taken into account.
   *
   ...
   * only the local date will be used. This means that any parsed time, time-zone or offset field is
   ...
   *
   * <p>This will parse the text fully according to the formatter, using the UTC zone. Once parsed,
   * only the local time will be used. This means that any parsed date, time-zone or offset field is
   * completely ignored. It also means that the zone and offset-parsed settings are ignored.
   *
   * @param text the text to parse, not null
