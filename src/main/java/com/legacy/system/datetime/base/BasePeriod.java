...
import com.legacy.system.datetime.Duration;
import com.legacy.system.datetime.DurationFieldType;
import com.legacy.system.datetime.MutablePeriod;
...
import com.legacy.system.datetime.convert.PeriodConverter;
import com.legacy.system.datetime.field.FieldUtils;
import java.io.Serializable;

/**
 * BasePeriod is an abstract implementation of ReadablePeriod that stores data in a <code>PeriodType
 ...
 *
 * <p>BasePeriod subclasses may be mutable and not thread-safe.
 *
...
public abstract class BasePeriod extends AbstractPeriod implements ReadablePeriod, Serializable {
  ...
  protected BasePeriod(
    ...
    iValues =
        setPeriodInternal(
            years, months, weeks, days, hours, minutes, seconds, millis); // internal method
  }
  ...
  /**
   * Creates a period from the given millisecond duration with the standard period type and ISO
   * rules, ensuring that the calculation is performed with the time-only period type.
   *
  ...
  /**
   * Creates a period from the given millisecond duration, which is only really suitable for
   * durations less than one day.
   *
   * <p>Only fields that are precise will be used. Thus the largest precise field may have a large
   * value.
   *
   * @param duration the duration, in milliseconds
   * @param type which set of fields this period supports, null means standard
  ...
  // -----------------------------------------------------------------------
  /**
   * Validates a period type, converting nulls to a default value and checking the type is suitable
   * for this instance.
   *
   ...
   *
   * <p>This method adds the period to the specified instant in order to calculate the duration.
   *
   * <p>An instant must be supplied as the duration of a period varies. For example, a period of 1
   * month could vary between the equivalent of 28 and 31 days in milliseconds due to different
   * length months. Similarly, a day can vary at Daylight Savings cutover, typically between 23 and
   * 25 hours.
   *
   ...
   *
   * <p>This method subtracts the period from the specified instant in order to calculate the
   * duration.
   *
   * <p>An instant must be supplied as the duration of a period varies. For example, a period of 1
   * month could vary between the equivalent of 28 and 31 days in milliseconds due to different
  ...
  // -----------------------------------------------------------------------
  /**
   * Checks whether a field type is supported, and if so adds the new value to the relevant index in
   * the specified array.
   *
