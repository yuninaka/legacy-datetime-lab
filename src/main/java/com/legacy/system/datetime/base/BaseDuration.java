...
import com.legacy.system.datetime.Interval;
import com.legacy.system.datetime.Period;
import com.legacy.system.datetime.PeriodType;
import com.legacy.system.datetime.ReadableDuration;
import com.legacy.system.datetime.ReadableInstant;
import com.legacy.system.datetime.convert.ConverterManager;
import com.legacy.system.datetime.convert.DurationConverter;
import com.legacy.system.datetime.field.FieldUtils;
import java.io.Serializable;

/**
 * BaseDuration is an abstract implementation of ReadableDuration that stores data in a <code>long
 * </code> duration milliseconds field.
 *
 * <p>This class should generally not be used directly by API users. The {@link ReadableDuration}
 * interface should be used when different kinds of duration objects are to be referenced.
 *
 * <p>BaseDuration subclasses may be mutable and not thread-safe.
 *
...
public abstract class BaseDuration extends AbstractDuration
  ...
  // -----------------------------------------------------------------------
  /**
   * Converts this duration to a Period instance using the specified period type and the ISO
   * chronology.
   *
   * <p>Only precise fields in the period type will be used. At most these are hours, minutes,
   * seconds and millis - the period type may restrict the selection further.
   *
   * <p>For more control over the conversion process, you must pair the duration with an instant,
   * see {@link #toPeriodFrom(ReadableInstant, PeriodType)}.
   *
  ...
  /**
   * Converts this duration to a Period instance using the standard period type and the specified
   * chronology.
   *
   * <p>Only precise fields in the period type will be used. Exactly which fields are precise
   * depends on the chronology. Only the time fields are precise for ISO chronology with a time
   * zone. However, ISO UTC also has precise days and weeks.
   *
   * <p>For more control over the conversion process, you must pair the duration with an instant,
   * see {@link #toPeriodFrom(ReadableInstant)} and {@link #toPeriodTo(ReadableInstant)}
   *
   ...
   *
   * <p>For more control over the conversion process, you must pair the duration with an instant,
   * see {@link #toPeriodFrom(ReadableInstant, PeriodType)} and {@link #toPeriodTo(ReadableInstant,
   * PeriodType)}
   *
   * @param type the period type to use, null means standard
   * @param chrono the chronology to use, null means ISO default
  ...
  /**
   * Converts this duration to a Period instance by adding the duration to a start instant to obtain
   * an interval using the standard period type.
   *
   * <p>This conversion will determine the fields of a period accurately. The results are based on
   * the instant millis, the chronology of the instant, the standard period type and the length of
   * this duration.
   *
   ...
   *
   * <p>This conversion will determine the fields of a period accurately. The results are based on
   * the instant millis, the chronology of the instant, the period type and the length of this
   * duration.
   *
   * @param startInstant the instant to calculate the period from, null means now
   * @param type the period type determining how to split the duration into fields, null means All
   *     type
   * @return a Period created using the millisecond duration from this instance
   */
  ...
  /**
   * Converts this duration to a Period instance by subtracting the duration from an end instant to
   * obtain an interval using the standard period type.
   *
   * <p>This conversion will determine the fields of a period accurately. The results are based on
   * the instant millis, the chronology of the instant, the standard period type and the length of
   * this duration.
