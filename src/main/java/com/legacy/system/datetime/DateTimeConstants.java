...
/**
 * DateTimeConstants is a non-instantiable class of constants used in the date time system. These
 * are the ISO8601 constants, but should be used by all chronologies.
 *
 * <p>DateTimeConstants is thread-safe and immutable.
 *
...
public class DateTimeConstants {
   ...
   *
   * <p>All new chronologies with different Era values should try to assign eras as follows. The era
   * that was in force at 1970-01-01 (ISO) is assigned the value 1. Earlier eras are assigned
   * sequentially smaller numbers. Later eras are assigned sequentially greater numbers.
   */
  ...
  /**
   * Hours in a typical day (24) (ISO). Due to time zone offset changes, the number of hours per day
   * can vary.
   */
  public static final int HOURS_PER_DAY = 24;

  /**
   * Minutes in a typical day (ISO). Due to time zone offset changes, the number of minutes per day
   * can vary.
   */
  public static final int MINUTES_PER_DAY = MINUTES_PER_HOUR * HOURS_PER_DAY;

  /**
   * Seconds in a typical day (ISO). Due to time zone offset changes, the number of seconds per day
   * can vary.
   */
  public static final int SECONDS_PER_DAY = SECONDS_PER_HOUR * HOURS_PER_DAY;

  /**
   * Milliseconds in a typical day (ISO). Due to time zone offset changes, the number of
   * milliseconds per day can vary.
   */
  ...
  /**
   * Hours in a typical week. Due to time zone offset changes, the number of hours per week can
   * vary.
   */
  public static final int HOURS_PER_WEEK = HOURS_PER_DAY * DAYS_PER_WEEK;

  /**
   * Minutes in a typical week (ISO). Due to time zone offset changes, the number of minutes per
   * week can vary.
   */
  public static final int MINUTES_PER_WEEK = MINUTES_PER_DAY * DAYS_PER_WEEK;

  /**
   * Seconds in a typical week (ISO). Due to time zone offset changes, the number of seconds per
   * week can vary.
   */
  public static final int SECONDS_PER_WEEK = SECONDS_PER_DAY * DAYS_PER_WEEK;

  /**
   * Milliseconds in a typical week (ISO). Due to time zone offset changes, the number of
   * milliseconds per week can vary.
   */
