...
/**
 * Defines an instant in the datetime continuum. This interface expresses the datetime as
 ...
 *
 * <p>Methods in your application should be defined using <code>ReadableInstant</code> as a
 * parameter if the method only wants to read the instant without needing to know the specific
 * datetime fields.
 *
 * <p>The {@code compareTo} method is no longer defined in this class in version 2.0. Instead, the
 * definition is simply inherited from the {@code Comparable} interface. This approach is necessary
 * to preserve binary compatibility. The definition of the comparison is ascending order by
 * millisecond instant. Implementors are recommended to extend {@code AbstractInstant} instead of
 * this interface.
 *
...
public interface ReadableInstant extends Comparable<ReadableInstant> {
   ...
   *
   * <p>The {@link Chronology} provides conversion from the millisecond value to meaningful fields
   * in a particular calendar system.
   *
   ...
   *
   * <p>This can be useful if you don't trust the implementation of the interface to be
   * well-behaved, or to get a guaranteed immutable object.
   *
   ...
   * and the Chronology. All ReadableInstant instances are accepted.
   *
   * <p>To compare two instants for absolute time (ie. UTC milliseconds ignoring the chronology),
   * use {@link #isEqual(ReadableInstant)} or {@link #compareTo}.
   *
   * @param readableInstant a readable instant to check against
