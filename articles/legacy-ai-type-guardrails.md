...
前回の記事「OSSライブラリで検証：仕様書ゼロからClaude Codeでテスト網羅率93%を達成した話」から、実在する成熟したJavaライブラリ **Joda-Time**（Java 5をターゲットにした150以上のソースファイルからなる実ライブラリ）を題材に、「仕様書ゼロ・テストゼロのレガシーコードを、AIエージェントにどこまで自律的に立て直せるか」という検証を続けています。
